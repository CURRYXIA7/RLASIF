...
data_dir = " "  # pdb files dir
in_ply = " " # Ply files dir
save_dir = " " # Save fingerprint files dir
...
learning_obj = RNA_site(
    ...
    n_rotations=8,   
    idx_gpu="/gpu:0",
    feat_mask= [1.0, 1.0, 1.0], #params["feat_mask"],
...
for pdb_id in pdb_ids:
    number = number + 1
    if number >= 1:
        in_dir = '../data_prepartion/data_preparation/' + data_dir + pdb_id + '/'
        for pid in pids:
            try:
